...
class MetadataReader(object):
    ...
    def facts_file_path(self, problem_name):
        raise NotImplementedError("This method must be implemented.")
...
class JSONMetadataReader(MetadataReader):
    ...
    def facts_file_path(self, problem_name):
        facts_file = self.grading_config[self.__problems_key][problem_name][self.__facts_file_key]
        facts_dir = self.grading_config[self.__facts_dir_key]
        facts_file_path = os.path.join(facts_dir, facts_file)
        return self.__resolve_path(facts_file_path)
